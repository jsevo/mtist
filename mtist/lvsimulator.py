...
class LV(object):
    ...
    def add_n_random_species(self,
        ...
        mean_interactions_magnitude = np.mean(
            np.mean(np.abs(all_interactions)))
        ...
        species_style['zz_random_sp'] = {'c': '#03719c', 's': '-', 'w': .75}
        self.ecosystem = ecosystem
        self.growth_rates = growth_rates
    ...
    def remove_random_species(self):
        ...
        self.ecosystem = dict([(k, drop_rand(core_eco[k])) for k in core_eco.keys()
                          if 'zz' not in k])
    ...
    def get_ecosystem_as_df(self):
        ...
        for i, focalspecies in enumerate(self.species_names):
            A_full.loc[focalspecies][:] = np.append(
                self.growth_rates[focalspecies], self.A[i])
        return (A_full)
    ...
    def plot_lv(self, t=None, y=None, yinit=[], savepath=None, figure=None, axes=None):
        ...
        fig, ax1, ax2 = util.plot_lv(t, y, self.ecosystem, self.species_style,
                                     yinit, figure=figure, axes=axes)
        if savepath:
    ...
    def simple_plot_lv(self, t=None, y=None, yinit=[], savepath=None):
        ...
        if len(yinit)==0:
            t, y, sol_t_all, sol_y_all, yinit = self.run_lv()
        fig, ax1 = util.simple_plot_lv(t, y, self.ecosystem, self.species_style,
                                     yinit)
        if savepath:
            fig.savefig(savepath, bbox_inches='tight', dpi=300)
        return (fig, ax1)
    ...
    def plot_ecosystem(self,
        ...
        if rename_species:
            if type(rename_species) == dict:
                rename_dict = rename_specie
            else:
                rename_dict = dict([(z, 'rs' + z[-4::])
                                    if 'zz' in z else ('mu', '$\mu$')
                                    for z in A_true.columns])
        ...
        axl.set_yticklabels(
            ['-15', '-8', '-2', '0', '2', '4', '6'], fontdict=util.font_ticks)
