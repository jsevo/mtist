...
# It was used for testing the "old" order of sampling the master dataset.
...
from mtist.graphing_utils import despine, savefig, easy_subplots


class ALT_ASSEMBLE_MTIST_DEFAULTS:
    RANDOM_SEED = 8927
    seq_depth_th = 0.01
...
### GLOBAL VARIABLES ###
# RNG = ALT_ASSEMBLE_MTIST_DEFAULTS.RNG
...
def assemble_mtist():
    ...
    for name, noise, nts, ss, sf in full_conditions_dict.keys():
        ...
        # Initiate rng
        cur_rng = np.random.default_rng(ALT_ASSEMBLE_MTIST_DEFAULTS.RANDOM_SEED)
        morning_of_each_day = np.linspace(0, 99, 25, dtype=int)

        # To regenerate the old way

        seq_start_list = cur_rng.integers(0, 10, size=50)

        random_idx_list_5 = [
            sorted(cur_rng.choice(morning_of_each_day, size=5, replace=False)) for _ in range(50)
        ]
        random_idx_list_10 = [
            sorted(cur_rng.choice(morning_of_each_day, size=10, replace=False)) for _ in range(50)
        ]
        random_idx_list_15 = [
            sorted(cur_rng.choice(morning_of_each_day, size=15, replace=False)) for _ in range(50)
        ]

        # Start sampling
        df_sampled = pd.DataFrame([])
        for i, (tid, subset) in enumerate(df.groupby("timeseries_id")):
            ...
            if ss == "even":
                ...
                if sf == 5:
                    idx = random_idx_list_5[i]
                elif sf == 10:
                    idx = random_idx_list_10[i]
                elif sf == 15:
                    idx = random_idx_list_15[i]

            elif ss == "seq":
                cur_start = seq_start_list[i]
                idx = gen_seq_idx(sf, cur_start)
    ...
    meta.to_csv("mtist_metadata.csv")
...
def implement_low_seq_depth():
    ...
    for fn in fns:
        ...
        mask = abun.apply(lambda r: r / r.sum(), axis=1) < ALT_ASSEMBLE_MTIST_DEFAULTS.seq_depth_th
