...
def make_full_dlogxdt(df, key, timekey, columns, specieskey='species'):
    ...
    for i, (v, g) in enumerate(df.groupby(key)):
        ...
        dlogxdt = gsorted[columns]\
            .apply(np.log)\
            .diff()\
            .apply(lambda x: np.divide(x,dt)).T.stack()
        # get the geommetric means of species abundances between timepoints
        geomm = gsorted[columns]\
        ...
        tmp = pd.DataFrame(tmp).reset_index()\
            .rename(columns={'level_0':'timeinterval',
                             'level_1': specieskey,
                             0:'dlogxdt'})
        tmp[key] = np.repeat(v, len(tmp))
        geomm = geomm.unstack().T.reset_index()\
            .rename(columns={'index':'timeinterval'})
        _dlogxdt = pd.merge(tmp, geomm, how='left', on='timeinterval')
        # append
        full_dlogxdt = full_dlogxdt.append(_dlogxdt, ignore_index=True)
    full_dlogxdt = full_dlogxdt.replace([np.inf, -np.inf], np.nan).dropna(
        how='any')
    return (full_dlogxdt)
